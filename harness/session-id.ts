...
export interface ResolveSessionIdOptions {
  /** Project root (nearest .git/.pi ancestor) — base is <root>/.pi/messenger. */
  projectRoot: string;
  /** Caller pi process pid (from findCallerPid). Undefined when not resolvable. */
  callerPid?: number;
}

function readTrim(filePath: string): string | undefined {
  try {
 ...
 */
export function resolveSessionId(options: ResolveSessionIdOptions): string | undefined {
