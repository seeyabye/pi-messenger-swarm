 ...
 */
export function getLiveSessionIds(dirs: Dirs): Set<string> {
 ...
 */
export function pruneOrphanedSessionChannels(
  state: MessengerState,
  dirs: Dirs,
  cwd: string,
  options?: { dryRun?: boolean; currentSessionId?: string }
): PruneResult {
  ...
  for (const channel of listChannels(dirs)) {
    if (channel.type !== 'session') continue;
