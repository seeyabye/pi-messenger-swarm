 ...
 */
function isAgentActive(cwd, agentName) {
 ...
 */
function isSpawnedAgentVouchedFor(cwd, sessionId, agentName) {
    const spawned = findSpawnedAgentByName(cwd, sessionId, agentName);
    if (!spawned)
        return false;
    if (spawned.status !== 'running')
        return false;
    if (spawned.pid && !isProcessAlive(spawned.pid))
        return false;
    return true;
}
/**
 ...
 */
export function cleanupStaleTaskClaims(cwd, sessionId) {
    ...
    for (const task of tasks) {
        ...
        if (active === false) {
            ...
            logFeedEvent(cwd, claimant, 'task.reset', task.id, 'agent crashed - task auto-unclaimed', task.channel ?? 'unknown');
            cleaned++;
        }
            ...
            logFeedEvent(cwd, claimant, 'task.reset', task.id, 'agent left - task auto-unclaimed', task.channel ?? 'unknown');
            cleaned++;
        }
    }
    return cleaned;
