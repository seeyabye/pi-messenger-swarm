...
function appendEvent(cwd, sessionId, event) {
 ...
 */
export function loadSpawnedAgents(cwd, sessionId) {
    const filePath = getAgentEventsJsonlPath(cwd, sessionId);
    ...
    return Array.from(agentsById.values()).sort((a, b) => Date.parse(b.startedAt) - Date.parse(a.startedAt));
}
export function getAgentEventHistory(cwd, sessionId, agentId) {
    const filePath = getAgentEventsJsonlPath(cwd, sessionId);
    if (!fs.existsSync(filePath))
        return [];
    const events = [];
...
function formatYamlMultiline(key, value) {
    if (value.includes('\n')) {
...
function generateAgentFile(cwd, sessionId, agent) {
...
function buildPrompt(request) {
    const objective = (request.objective ?? request.message ?? '').trim();
    const lines = ['# Mission Brief', '', objective];
    if (request.context?.trim())
        lines.push('', '## Additional Context', request.context.trim());
    if (request.taskId) {
        lines.push('', '## Task Execution Procedure', 'Follow this sequence when executing an assigned task:', '', '1. Claim the task before starting:', `   pi-messenger-swarm task claim ${request.taskId}`, '   If the claim fails, report the conflict and await instruction. Do not proceed with unclaimed work.', '', '2. Update progress at regular intervals:', `   pi-messenger-swarm task progress ${request.taskId} "Specific milestone achieved"`, '   Send updates every 3-5 tool calls or upon completing significant milestones. Include what was done and why.', '', '3. Mark the task done upon completion:', `   pi-messenger-swarm task done ${request.taskId} "Concrete accomplishment with evidence"`, '   Provide evidence of completion in the summary.');
    }
    lines.push('', '## Definition of Done', '- Objective addressed with concrete output.', request.taskId
        ? '- Progress updates recorded via pi-messenger-swarm at appropriate intervals.'
        : '', request.taskId ? '- Task marked done via pi-messenger-swarm before exit.' : '', '- All findings and evidence recorded in task progress/done (the coordinator reads output via `pi-messenger-swarm task show`, not your response text).', '- Any file reservations released before exit.', '- EXIT IMMEDIATELY after task.done: bash({ command: "exit 0" }). Do not idle or monitor after completion.');
    return lines.join('\n');
}
...
function createArgs(state, model) {
    ...
    if (state.systemPrompt.trim().length > 0) {
        ...
        args._promptTmpDir = promptTmpDir;
    }
...
function attachHandlers(proc, state, promptTmpDir, sessionId) {
    ...
    proc.on('close', (code, signal) => {
        ...
        if (state.channel) {
            const feedType = status === 'completed'
                ? 'spawn.completed'
                : status === 'failed'
                    ? 'spawn.failed'
                    : 'spawn.stopped';
            const taskLabel = runtime.record.taskId || state.id;
            const preview = status === 'completed'
...
export function spawnSubagent(cwd, request, sessionId, inheritedChannel, projectCwd) {
    ...
    const promptTmpDir = args._promptTmpDir;
...
export function listSpawned(cwd, sessionId, includeAll = false) {
    ...
    for (const [id, runtime] of runtimes.entries()) {
        if (runtime.record.cwd !== cwd)
            continue;
        if (runtime.record.sessionId !== sessionId)
            continue;
...
export function listSpawnedHistory(cwd, sessionId) {
    return listSpawned(cwd, sessionId, true);
}
export function findSpawnedAgentByName(cwd, sessionId, name) {
    const allAgents = listSpawnedHistory(cwd, sessionId);
    return allAgents.find((a) => a.name === name) ?? null;
}
export function stopSpawn(cwd, id) {
...
export function stopAllSpawned(cwd) {
    for (const [id, runtime] of runtimes.entries()) {
        if (cwd && runtime.record.cwd !== cwd)
            continue;
        if (runtime.detached) {
            if (runtime.record.pid && isProcessAlive(runtime.record.pid)) {
                runtime.stopping = true;
...
export function forceKillAllSpawned(cwd) {
...
export function cleanupExitedSpawned(cwd, sessionId) {
    let finalized = 0;
    for (const [id, runtime] of runtimes.entries()) {
        if (runtime.record.cwd !== cwd)
            continue;
...
export function reconcileSpawnedAgents(cwd, sessionId) {
    const persisted = loadSpawnedAgents(cwd, sessionId);
    let reconciled = 0;
...
export function getRunningSpawnCount(cwd) {
    let count = 0;
...
export function clearSpawnStateForTests() {
    runtimes.clear();
...
function getRuntimesFilePath(messengerDir) {
    return path.join(messengerDir, 'spawn-runtimes.json');
}
export function persistRuntimes(messengerDir) {
    const entries = [];
    for (const [id, runtime] of runtimes.entries()) {
 ...
 */
export function restoreRuntimes(messengerDir) {
    const filePath = getRuntimesFilePath(messengerDir);
    if (!fs.existsSync(filePath))
        return 0;
    let entries;
 ...
 */
function finalizeDeadPersistedRuntime(record) {
 ...
 */
export function reconcileAndRestoreOrphans(messengerDir) {
...
function restoreRuntimeEntries(entries) {
    let restored = 0;
    for (const entry of entries) {
        ...
        const fakeProcess = {
            ...
            off: () => fakeProcess,
            once: () => fakeProcess,
            removeAllListeners: () => fakeProcess,
            stdout: null,
            stderr: null,
            stdin: null,
        };
 ...
 */
function loadSpawnedAgentsFromFile(filePath) {
    if (!fs.existsSync(filePath))
        return [];
    const agentsById = new Map();
    const content = fs.readFileSync(filePath, 'utf-8');
    ...
    detachedPollTimer = setInterval(() => {
        for (const [id, runtime] of runtimes.entries()) {
            ...
            if (!isProcessAlive(runtime.record.pid)) {
                ...
                if (channel) {
                    const feedType = runtime.record.status === 'completed'
                        ? 'spawn.completed'
                        : runtime.record.status === 'stopped'
                            ? 'spawn.stopped'
                            : 'spawn.failed';
                    const preview = runtime.record.status === 'completed'
                        ? runtime.record.objective || undefined
                        : runtime.record.error || undefined;
                    logFeedEvent(runtime.record.projectCwd || runtime.record.cwd, agentName, feedType, taskLabel, preview, channel);
                }
                removeLiveWorker(runtime.record.cwd, runtime.record.taskId || spawnLiveKey(id));
            }
        }
        // Stop polling if no detached runtimes remain
        const hasDetached = Array.from(runtimes.values()).some((r) => r.detached && r.record.status === 'running');
 ...
 */
export function clearPersistedRuntimes(messengerDir) {
    const filePath = getRuntimesFilePath(messengerDir);
    try {
        if (fs.existsSync(filePath))
