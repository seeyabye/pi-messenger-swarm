...
import { cleanupExitedSpawned, getRunningSpawnCount, listSpawned, listSpawnedHistory, reconcileSpawnedAgents, spawnSubagent, stopSpawn, } from '../spawn.js';
import { formatRoleLabel } from '../labels.js';
export function executeSpawn(op, params, state, cwd, sessionId, maxConcurrentSpawns, callerCwd) {
...
function spawnList(cwd, sessionId) {
...
function spawnHistory(cwd, sessionId) {
    ...
    const lines = ['# Spawned Agent History', ''];
    if (running.length > 0) {
        lines.push('## Running');
...
function spawnCreate(params, state, cwd, sessionId, maxConcurrentSpawns, callerCwd) {
    ...
    if (!params.taskId && !params.force) {
        ...
        if (ready.length > 0) {
            ...
            return result(`⚠️  You have ${ready.length} ready task${ready.length === 1 ? '' : 's'} waiting to be claimed.\n${list}\n\n` +
    ...
    if (running >= limit) {
        return result(`Error: ${running} subagent${running === 1 ? '' : 's'} already running (limit: ${limit}). ` +
