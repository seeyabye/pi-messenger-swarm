...
export function normalizeJoinedChannels(channels, currentChannel, sessionChannel) {
    const set = new Set();
    for (const channel of channels ?? []) {
        if (channel)
            set.add(normalizeChannelId(channel));
    }
    if (sessionChannel)
        set.add(normalizeChannelId(sessionChannel));
    if (currentChannel)
        set.add(normalizeChannelId(currentChannel));
    set.add(MEMORY_CHANNEL_ID);
    return Array.from(set);
}
export function keepNamedChannels(dirs, channels) {
    const kept = new Set();
    for (const channel of channels ?? []) {
        if (!channel)
            continue;
 ...
 */
export function getEffectiveSessionId(cwd, state) {
...
export function ensureStateChannels(state, dirs, ctx, options) {
    ...
    if (resetToSessionChannel) {
        ...
        state.joinedChannels = normalizeJoinedChannels(keepNamedChannels(dirs, state.joinedChannels), state.currentChannel, state.sessionChannel);
        return;
    }
...
export function applyRegistrationDefaults(reg) {
...
export function updateChannelsInRegistration(state, reg) {
    return {
        ...reg,
