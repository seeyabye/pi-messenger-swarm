 ...
 */
export function installShellAlias() {
    try {
...
export function createHarnessServer(messengerDir) {
    ...
    function start() {
        ...
        if (process.env.PI_SWARM_SPAWNED === '1')
            return;
        const { PI_MESSENGER_CHANNEL, ...restEnv } = process.env;
