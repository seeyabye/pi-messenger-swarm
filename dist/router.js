...
export async function executeAction(action, params, state, dirs, ctx, deliverMessage, updateStatus, _appendEntry, config, _signal) {
    ...
    if (group === 'join') {
        return handlers.executeJoin(state, dirs, ctx, deliverMessage, updateStatus, params.spec, config?.nameTheme, config?.feedRetention, params.channel, params.create);
    }
    ...
    switch (group) {
        ...
        case 'feed':
            return handlers.executeFeed(cwd, requireChannel(), params.limit, config?.swarmEventsInFeed ?? true, params.channel);
        case 'send':
            return handlers.executeSend(state, dirs, cwd, params.to, params.message, params.replyTo, params.channel ?? requireChannel());
        case 'broadcast':
            return result('Action "broadcast" was removed. Use `pi-messenger-swarm send #channel "message"` instead.', { mode: 'broadcast_removed', error: 'removed_action', action });
        case 'reserve':
        ...
        case 'task': {
            ...
            return executeTask(operation, params, state, cwd, params.channel ?? requireChannel(), sessionId);
        }
        ...
        case 'claim': {
            ...
            return executeTask('claim', { ...params, id: taskId }, state, cwd, params.channel ?? requireChannel(), sessionId);
        }
        case 'unclaim': {
            const taskId = params.taskId ?? params.id;
            if (!taskId) {
            ...
            return executeTask('unclaim', { ...params, id: taskId }, state, cwd, params.channel ?? requireChannel(), sessionId);
        }
        case 'complete': {
            const taskId = params.taskId ?? params.id;
            if (!taskId) {
            ...
            return executeTask('done', { ...params, id: taskId }, state, cwd, params.channel ?? requireChannel(), sessionId);
        }
        case 'channels':
        ...
        case 'channel': {
            ...
            if (operation === 'delete') {
                return handlers.executeChannelDelete({ channel: params.channel, force: params.force === true }, state, dirs, cwd);
            }
        ...
        case 'spawn':
            return executeSpawn(op, params, state, cwd, sessionId, config?.maxConcurrentSpawns, callerCwd);
        default:
