 ...
 */
export function executeChannelPrune(
  state: MessengerState,
  dirs: Dirs,
  cwd: string,
  dryRun = false
) {
  if (!state.registered) return notRegisteredError();
  ...
  if (kept.length > 0) {
    lines.push(
      `Kept ${kept.length} session channel${kept.length === 1 ? '' : 's'} (active or with history).`
    );
  }
 ...
 * content.
 */
export function executeChannelDelete(
  params: { channel?: string; force?: boolean },
  state: MessengerState,
  dirs: Dirs,
  cwd: string
) {
  ...
  if (id === MEMORY_CHANNEL_ID) {
    return result(
      `Error: ${displayChannelLabel(id)} is a protected channel and cannot be deleted.`,
      { mode: 'channel.delete', error: 'protected', channel: id }
    );
  }
  ...
  if (state.joinedChannels.includes(id)) {
    return result(
      `Error: you are currently joined to ${displayChannelLabel(id)}. Switch channels first.`,
      { mode: 'channel.delete', error: 'self_joined', channel: id }
    );
  }
  ...
  if (joined.length > 0) {
    ...
    return result(
      `Error: cannot delete ${displayChannelLabel(id)} — ${joined.length} agent${joined.length === 1 ? '' : 's'} joined: ${names}. Have them switch channels first.`,
      { mode: 'channel.delete', error: 'agents_joined', channel: id, agents: names }
    );
  }
  ...
  if (eventCount > 0 && !params.force) {
    return result(
      `Error: ${displayChannelLabel(id)} has ${eventCount} feed event${eventCount === 1 ? '' : 's'}. Re-run with --force to delete it anyway.`,
      { mode: 'channel.delete', error: 'has_history', channel: id, eventCount }
    );
  }
